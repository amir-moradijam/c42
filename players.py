...
class VLLMAgentPlayer(GPTAgentPlayer):
    def __init__(self, name: str, model: str = "Parsenal/granite_c4", temperature: float = 0.3, max_tokens: int = 1024, base_url: str = "http://localhost:8000/v1"):
    ...
    def get_move(self, board: Board) -> int:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "GAME_MODE=CONNECT_FOUR <top1><optimal_move>"},
